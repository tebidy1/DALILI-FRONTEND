  ...
  const steps = guide.steps
    .map((s) => {
      ...
      if (s.block === 'link') {
        ...
        let href = ''
        // فحص الأمان هنا مكرر عمدًا عن safeHref في rich-text: الاختبار يثبّت الرابط خامًا لا مطبَّعًا —
        // لا يُدمَج إلا بتغيير العقد (فحص 2026-09-21).
        try {
          const u = new URL(raw)
