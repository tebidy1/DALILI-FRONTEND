...
describe('لوحة الحبر الموحّدة — خمسة ألوان للهدف والشرح معًا (قرار المالك)', () => {
  ...
  it('يقبل لونًا من اللوحة ويرفض ما عداه — لا لون حرّ يتسرّب للنموذج', () => {
    expect(isMarkColor('#16a34a')).toBe(true)
    expect(isMarkColor('#00ff00')).toBe(false)
    expect(isMarkColor('')).toBe(false)
  })
})
...
describe('شكل إطار الهدف — مستطيل أو بيضاوي', () => {
  ...
  it('isMarkShape يقبل الشكلين ويرفض ما عداهما', () => {
    expect(isMarkShape('rect')).toBe(true)
    expect(isMarkShape('ellipse')).toBe(true)
    expect(isMarkShape('oval')).toBe(false)
    expect(isMarkShape('')).toBe(false)
  })
