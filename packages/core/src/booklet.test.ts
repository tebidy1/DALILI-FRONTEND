...
describe('bookletText — النص القابل للفهرسة', () => {
  it('يضم عناوين الكتل ونص الكتل المنسّقة معًا', () => {
    const steps = [
      step({ id: 'h', block: 'header', title: 'الإغلاق الشهري' }),
      step({ id: 't', block: 'text', rich: [{ para: 'p', runs: [{ text: 'راجع القيود' }] }] }),
    ]
    expect(bookletText(steps)).toBe('الإغلاق الشهري راجع القيود')
  })

  it('كرّاسة فارغة تعطي سلسلة فارغة', () => {
    expect(bookletText([])).toBe('')
  })
})
