...
pub fn queue_guide(root: &Path, session_id: &str, body_text: &str) -> Result<QueuedGuide, String> {
...
mod tests {
    ...
    fn الاضافة_تكتب_الجسم_والسجل_بلا_ملف_معلق() {
        ...
        let rec = queue_guide(&t.root, "s-9", body).unwrap();
    ...
    fn الجسم_المعتم_النص_الاعتباطي_يكتب_بلا_فك_وقراءة_سجل_فاسد_تتجاهل() {
        ...
        queue_guide(&t.root, "s-1", "هذا ليس JSON{{{").unwrap();
        assert_eq!(
    ...
    fn الاضافة_بمعرّف_فاسد_ترد_خطأ_ولا_تكتب_شيئا() {
        let t = Tmp::new("badid");
        assert!(queue_guide(&t.root, "../evil", "{}").is_err());
        assert!(queue_guide(&t.root, "s-", "{}").is_err());
    ...
    fn القوائم_تفصل_مرتبة_والفشل_يبقي_الجسم_والإعادة_تصفر_والحذف_يمسح_الاثنين() {
        ...
        queue_guide(&t.root, "s-1", "ب1").unwrap();
        queue_guide(&t.root, "s-2", "ب2").unwrap();
        queue_guide(&t.root, "s-3", "ب3").unwrap();
