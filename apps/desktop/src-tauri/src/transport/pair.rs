...
/// بثّ فقد الاقتران — السبب العربيّ وحده ما يعبّر، ولا شيء آخر
fn lost<R: tauri::Runtime>(app: &tauri::AppHandle<R>, reason_ar: &str) {
    let _ = app.emit("auth://lost", serde_json::json!({ "reasonAr": reason_ar }));
}
...
fn poll_loop<R: tauri::Runtime>(
    ...
    loop {
        ...
        if started.elapsed() >= ttl {
            lost(&app, "انتهت مهلة الربط — ابدأ من جديد");
            return;
        }
                    ...
                    .and_then(parse_json::<TokenOk>)
                {
                    ...
                    Ok(tok) => {
                            ...
                            .and_then(|_| vault::store_email(&origin, &tok.user.email))
                        {
                            ...
                            Err(e) => {
                                lost(&app, &format!("تعذّر حفظ الرمز آمنًا: {e}"));
                            }
                        }
                    }
                    Err(_) => {
                        lost(&app, "ردّ اقتران غير مفهوم");
                    }
                }
                return;
            }
