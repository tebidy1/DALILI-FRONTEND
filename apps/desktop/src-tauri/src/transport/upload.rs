...
pub fn process_pending<S: UploadSink + ?Sized>(
        ...
        .build()
    {
        Ok(c) => c,
        Err(_) => {
            emit_status(sink, root, guides_root, Some("تهيئة عميل الشبكة فشلت"));
            return report;
        }
    };
    let url = match pair::http_url(origin, "/api/uploads") {
        Ok(u) => u,
        Err(e) => {
            emit_status(sink, root, guides_root, Some(&e));
            return report;
        }
    };
    for item in queue::list_pending(root) {
        let mut rec = item;
        let key = idempotency_key(&rec.session_id, &rec.local_id);
        let mut rounds = 0u32;
        loop {
            ...
            let Some(token) = (io.token_of)(origin) else {
                ...
                report.stopped = true;
                emit_status(sink, root, guides_root, None);
                return report;
            };
            let shot = match queue::queued_shot_path(root, &rec.local_id) {
                Ok(p) => p,
                Err(e) => {
                    emit_status(sink, root, guides_root, Some(&e));
                    return report;
            ...
            let (status, body) = match upload_once(&client, &url, &token, &key, &shot) {
                UploadReply::Status(s, b) => (s, b),
                UploadReply::Network => (0, String::new()),
            };
            let mut step = next_step(policy::classify_response(status), rec.attempt);
            if step == ItemStep::Finish {
                // الجسم يُحلَّ قبل الثقة بالنجاح — ‏200 بلا معرّف لا يحذف شيئًا
                match serde_json::from_str::<UploadOk>(&body) {
                    Ok(ok) => {
                        if let Err(e) = queue::remove_item(root, &rec.local_id) {
                            emit_status(sink, root, guides_root, Some(&e));
                            break;
                        }
                    ...
                    Err(_) => step = next_step(policy::Action::Backoff, rec.attempt),
                }
            }
            match step {
                ...
                ItemStep::Fail { error_ar } => {
                    ...
                    emit_status(sink, root, guides_root, Some(&error_ar));
                    break;
                }
                ItemStep::Retry { delay_ms, grow_attempt } => {
                    if grow_attempt {
                        rec.attempt += 1;
                        let _ = queue::save_attempt(root, &rec, rec.attempt);
                    }
                    emit_status(sink, root, guides_root, None);
...
pub fn process_guides<S: UploadSink + ?Sized>(
        ...
        .build()
    {
        Ok(c) => c,
        Err(_) => {
            emit_status(sink, files_root, root, Some("تهيئة عميل الشبكة فشلت"));
            return report;
        }
    };
    let url = match pair::http_url(origin, "/api/guides") {
        Ok(u) => u,
        Err(e) => {
            emit_status(sink, files_root, root, Some(&e));
            return report;
        }
    };
    for item in guide_queue::list_pending(root) {
        let mut rec = item;
        let key = guide_queue::guide_key(&rec.session_id);
        let mut rounds = 0u32;
        loop {
            if rounds >= max_rounds {
                break; // ما زال منتظِرًا — الدورة القادمة تكمل من عدّاده المحفوظ
            ...
            let Some(token) = (io.token_of)(origin) else {
                report.stopped = true;
                emit_status(sink, files_root, root, None);
                return report;
            };
            // الجسم من القرص كما هو — فقدُه فشلٌ نهائيّ صادق لا عابر يُرمق
            let body_file = match guide_queue::queued_body_path(root, &rec.session_id) {
                Ok(p) => p,
                Err(e) => {
                    emit_status(sink, files_root, root, Some(&e));
                    return report;
            ...
            let body_text = match std::fs::read_to_string(&body_file) {
                Ok(t) => t,
                Err(_) => {
                    let err = "فقد جسم الدليل من الطابور";
                    let _ = guide_queue::mark_failed(root, &rec, err);
                    report.failed += 1;
                    emit_status(sink, files_root, root, Some(err));
                    break;
                }
            };
            let (status, body) =
                match upload_json_once(&client, &url, &token, &key, &body_text) {
                    UploadReply::Status(s, b) => (s, b),
                    UploadReply::Network => (0, String::new()),
                };
            let action = policy::classify_response(status);
            // فشل الدليل بمصطلحه — البقيّة من السلّم المشترك
            let mut step = match action {
                policy::Action::Failed => ItemStep::Fail {
                    error_ar: "رفض الخادم الدليل — بنية غير صالحة".into(),
                },
                other => next_step(other, rec.attempt),
            };
            if step == ItemStep::Finish {
                ...
                match guide_id {
                    Some(guide_id) => {
                        if let Err(e) = guide_queue::remove_item(root, &rec.session_id) {
                            emit_status(sink, files_root, root, Some(&e));
                            break;
                        }
                    ...
                    None => step = next_step(policy::Action::Backoff, rec.attempt),
                }
            }
            match step {
                ItemStep::Finish => unreachable!("النجاح حُدِّث أعلاه"),
                ItemStep::Fail { error_ar } => {
                    ...
                    report.failed += 1;
                    emit_status(sink, files_root, root, Some(&error_ar));
                    break;
                }
                ItemStep::Retry { delay_ms, grow_attempt } => {
                    if grow_attempt {
                        rec.attempt += 1;
                        let _ = guide_queue::save_attempt(root, &rec, rec.attempt);
                    }
                    emit_status(sink, files_root, root, None);
                    (io.sleep)(Duration::from_millis(delay_ms));
                    continue;
...
mod tests {
    ...
    fn الدليل_الناجح_يحذف_العنصر_ويبث_المعرّف_والجسم_يعبر_حرفيًا() {
        ...
        guide_queue::queue_guide(&g, "s-it", GUIDE_BODY).unwrap();
        let token = Mutex::new(Some("itq_tok".to_string()));
        let forgotten = AtomicBool::new(false);
        let sleeps = Mutex::new(Vec::new());
        let sink = RecSink::default();
