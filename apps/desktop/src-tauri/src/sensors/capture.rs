...
fn frame_path(seq: u64) -> String {
    crate::transport::queue::frames_dir()
...
// الدالة تحمل D3D والحلقة وانتظار السكون وقرار الالتقاط معًا — فكّ الالتقاط إلى answer_pick مجرَّد حركة كود؛
// الخيوط والتفكيك مسوَّرة قياسًا فلا تُمَس (فحص 2026-09-21).
        ...
        while let Ok(m) = rx.recv() {
            match m {
                CaptureMsg::Frame(f) => {
                    ...
                    LAST_ARRIVAL_MS.store(now_ms_u64(), std::sync::atomic::Ordering::Relaxed);
                ...
                CaptureMsg::Pick { t_hns, which, reply } => {
                    ...
                    let Some(slot) = pick_frame(&view, t_hns, which).map(|k| valid[k]) else {
                        if which == Which::After {
                            // TAM-FIX (2026-09-21) كشف السكون: لا إطار بعد النقرة
                            // ولا وصول منذ 120ms ⇐ الشاشة ساكنة يقينًا فانتظار
                            // الـ350ms مضيعة، والإجابة الفورية بإطار «قبل» صادقة
                            // بصريًّا (الشاشة لم تتغير). يعالج كذلك تدافع النقرات
                            // المتلاحقة: موضع الانتظار الواحد كان تلغيه كل نقرة
                            // جديدة فتتراكم اسقاطات 350ms = «التردّم» ببلاغ المالك
                            if after_should_fallback_instantly(
                                now_ms_u64(),
                                LAST_ARRIVAL_MS.load(std::sync::atomic::Ordering::Relaxed),
                            ) {
                                #[cfg(debug_assertions)]
                                eprintln!(
                                    "[capture] pick-after: static-fallback (quiet {}ms)",
                                    now_ms_u64().saturating_sub(LAST_ARRIVAL_MS.load(std::sync::atomic::Ordering::Relaxed))
                                );
                                match pick_frame(&view, t_hns, Which::Before).map(|k| valid[k]) {
                                    Some(bslot) => {
                                        let (w, h, bgra) = read_slot(&dev, &ctx, &desc, &slots[bslot]);
                                        let _ = reply.send(Some(CpuShot { hns: stamps[bslot], w, h, bgra, monitor: mon_info }));
                                    }
                                    None => { let _ = reply.send(None); }
                                }
                                continue;
                            }
...
/// قرار الإسقاط الفوري لطلب «بعد»: هدوء الوصول تجاوز 120ms ⇐ الشاشة ساكنة
/// والانتظار مضيعة. (يُستدعى بعد تحقّق خلوص الحلقة — لا يُصدر حكمًا على حلقة باردة)
fn after_should_fallback_instantly(now_ms: u64, last_arrival_ms: u64) -> bool {
    now_ms.saturating_sub(last_arrival_ms) > STATIC_QUIET_MS
}

/// TAM-FIX (2026-09-21): عدد الترميزات الكاملة الطائرة خلفيًّا — التسليم ينتظرها
/// قبل قراءة ملفات الإطارات كي لا يصل ملفٌ نصف مولود
static PENDING_ENCODERS: std::sync::atomic::AtomicU32 = std::sync::atomic::AtomicU32::new(0);

/// انتظار انتهاء كل ترميزات الخلفية (سقف زمني صدق) — true ⇐ صفاء كامل
pub fn wait_pending_encoders(timeout: std::time::Duration) -> bool {
    let t0 = std::time::Instant::now();
    while PENDING_ENCODERS.load(std::sync::atomic::Ordering::Relaxed) > 0 {
...
pub fn stop() {
    ...
    if let Some(h) = handle {
        // بلاغ المالك 2026-09-19 («جارٍ بناء الدليل» معلّقًا): ‏join غير محدود
        // على خيط تفكيكٍ عالق في ‏WinRT/GPU مزدحم جمّد ‏recording_stop كله
        // فعلق البطاقة للأبد. المهلة محدودة: بعدها يُترك الخيط ينتهي وحده
        // (ويندوز يجمعه عند الخروج) ومسارُ التسليم يكمل لا يُحتجز
        ...
        match done_rx.recv_timeout(Duration::from_secs(2)) {
            ...
            Err(_) => {
                #[cfg(debug_assertions)]
                eprintln!("[capture] teardown exceeded 2s — detaching thread, path continues");
            }
        }
    }
}
...
pub fn pick(seq: u64, which: Which) -> crate::sensors::events::FramePickResult {
    ...
    // TAM-DIAG (2026-09-21): تشريح زمن النقرة→صورة — «نقرة→دخول» يقيس انتظار
    // الجلسة (المجدول/النبضات) و«دخول→انتهاء» يقيس الكاميرا والقراءة والترميز
    let click_to_entry_ms = crate::sensors::clock::qpc_ms(crate::sensors::clock::qpc()) - crate::sensors::clock::qpc_ms(raw);
    ...
    let res = match answered {
        Ok(Some(shot)) => {
            ...
            // TAM-FIX (2026-09-21) تفكيك المسار: المصغّرة فورًا من المخزن (بضع
            // مئات م.ث — هي التي تُظهرها البطاقة)، والترميز الكامل ثقيلُ الحجم
            // (8+ ميغابكسل، وFP16 على شاشات HDR) خيطًا خلفيًّا لا يحجب البطاقة
            // ولا خيط الحلقة. التسليم ينتظر الترميزات في wait_pending_encoders
            let CpuShot { hns, w, h, bgra, monitor } = shot;
            let fm = FrameMonitor {
                x: monitor.x as f64,
                y: monitor.y as f64,
                w: monitor.w as f64,
                h: monitor.h as f64,
                dpi: monitor.dpi as f64,
            };
            // المصغّرة تستعير البكسلات نفسها التي يملكها الترميز لاحقًا — بلا
            // نسخة كاملة للإطار (‏8+ ميغابايت) على أمل المصغّرة وحدها
            let _ = crate::sensors::burn::write_thumb(&format!("f-{seq}"), w, h, &bgra);
            PENDING_ENCODERS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            let enc_t0 = std::time::Instant::now();
            let enc_path = path.clone();
            std::thread::spawn(move || {
                save_jpeg(&enc_path, &CpuShot { hns, w, h, bgra, monitor });
                #[cfg(debug_assertions)]
                eprintln!("[capture] encode seq={} done in {}ms", seq, enc_t0.elapsed().as_millis());
                PENDING_ENCODERS.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            });
    ...
    #[cfg(debug_assertions)]
    eprintln!(
        "[pick] seq={} click→entry={:.0}ms entry→done={}ms {}",
        seq,
        click_to_entry_ms,
        pick_t0.elapsed().as_millis(),
        match &res {
            FramePickResult::Picked(p) => format!("picked delta={:.1}ms", p.delta_ms),
            FramePickResult::Missing { missing } => format!("missing {missing:?}"),
        }
    );
    res
}
...
/// نافذة رسائل الفحص لبراهين الالتقاط الحيّة — كانت منسوخة في كل اختبار:
/// صنف ‏itqan-selftest-{title} أعلى الشاشة على خيط رسائل خاصّ، ‏protect ⇐
/// ‏WDA_EXCLUDEFROMCAPTURE، وتعيد (المقبض، الخيط) كي تُحسب نقطة النقر عليها
/// ويُفكّ الخيط بـWM_QUIT
#[cfg(test)]
fn spawn_msg_window(title: &'static str, x: i32, protect: bool) -> (isize, u32) {
    ...
    use windows::Win32::UI::WindowsAndMessaging::{
        ...
        SetWindowDisplayAffinity, TranslateMessage, MSG, WDA_EXCLUDEFROMCAPTURE, WNDCLASSW,
...
fn الالتقاط_الحي_يخرج_جبج_سليما() {
    ...
    // نافذة الفحص — بلا حماية: تعيد (المقبض، الخيط) كي تُحسب نقطة النقر عليها
    start();
    let (wh, wtid) = spawn_msg_window("dirt", 60, false); // إنشاؤها = تغيير مضمون ⇒ إطارات
...
fn البعد_على_السطح_الساكن_يسقط_لآخر_إطار_قبل_النقرة() {
    ...
    use windows::Win32::UI::WindowsAndMessaging::{
        GetWindowRect, PostThreadMessageW, WM_QUIT,
    };
    ...
    let (wh, wtid) = spawn_msg_window("static-after", 60, false);
    start();
...
fn الالتقاط_قبل_أول_إطار_لا_يسقط_خيط_الحلقة() {
    ...
    let (wh, wtid) = spawn_msg_window("pre-frame-pick", 60, false);
    let center = unsafe {
        let mut r = RECT::default();
        GetWindowRect(HWND(wh as *mut _), &mut r).unwrap();
        POINT { x: (r.left + r.right) / 2, y: (r.top + r.bottom) / 2 }
...
fn دورات_البدء_والايقاف_المتكررة_لا_تهوي_العملية() {
    ...
    let (wh, wtid) = spawn_msg_window("teardown-cycles", 60, false);
    let center = unsafe {
        let mut r = RECT::default();
        GetWindowRect(HWND(wh as *mut _), &mut r).unwrap();
        POINT { x: (r.left + r.right) / 2, y: (r.top + r.bottom) / 2 }
...
fn المحمية_بلا_لقطة_والحرّة_تُلتقط() {
    ...
    use windows::Win32::UI::WindowsAndMessaging::{
        GetWindowDisplayAffinity, GetWindowRect, PostThreadMessageW, WDA_EXCLUDEFROMCAPTURE,
        WM_QUIT,
    };
    ...
    let (ph, ptid) = spawn_msg_window("protected5", 60, true);
    let (ch, ctid) = spawn_msg_window("control5", 430, false);
    ...
    #[test]
    fn سكون_الشاشة_يجيب_فورا_بإطار_قبل() {
        // TAM-FIX 2026-09-21: هدوء الوصول فوق 120ms ⇐ اسقاط فوري لآخر «قبل»
        assert!(after_should_fallback_instantly(1000, 870), "هدوء 130ms ⇐ ساكنة");
        assert!(!after_should_fallback_instantly(1000, 970), "هدوء 30ms ⇐ قد يصل إطار، انتظر");
        assert!(!after_should_fallback_instantly(50, 40), "هدوء 10ms ⇐ انتظر");
    }

    #[test]
    fn نقرة_نافذة_عمليتنا_تُرشَّح_من_المصدر() {
        // قاعدة 2026-09-20: نقرات نوافذ الودجة لا تولّد إيماءات أصلًا —
        // المُرشِّح في الخطّاف يعرف نوافذنا بنسبتها لعمليتنا لا بالمستطيلات.
        ...
        let (_wh, wtid) = spawn_msg_window("selfwin9", 800, false);
        std::thread::sleep(std::time::Duration::from_millis(400));
        // مركز النافذة الفاحصة (800+160, 200+100): فوق نافذة من عمليتنا ⇐ تُرشَّح
        assert!(
            crate::sensors::hook::point_in_our_process(POINT { x: 960, y: 300 }),
            "نافذة عمليتنا تحت النقرة ⇐ تُرشَّح في المصدر"
        );
        // الركن الأعلى الأيسر ليس من نوافذنا ⇐ نقرة تعلّم شرعيّة تمرّ
        assert!(
            !crate::sensors::hook::point_in_our_process(POINT { x: 5, y: 5 }),
            "نقطة خارج نوافذنا ⇐ لا ترشيح"
        );
        unsafe {
            let _ = PostThreadMessageW(wtid, WM_QUIT, WPARAM(0), LPARAM(0));
        }
