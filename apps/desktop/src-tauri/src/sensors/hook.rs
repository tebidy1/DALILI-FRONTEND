...
use windows::Win32::Foundation::HWND; // اكتمال توقيع GetMessageW/PostThreadMessageW
...
/// آخر ختم إدخال — حالة المراقب المعتمَدة في الخطّة
static LAST_INPUT_QPC: AtomicI64 = AtomicI64::new(0);
...
fn offer(evt: InputEvt) {
    LAST_INPUT_QPC.store(qpc(), Ordering::SeqCst);
...
/// ختم آخر إدخال (QPC خام) — حالة المراقب المعتمَدة في الخطّة، ويقرؤها القياس الحيّ
pub fn last_input_qpc() -> i64 {
    LAST_INPUT_QPC.load(Ordering::SeqCst)
}
