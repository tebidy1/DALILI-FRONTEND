...
/// فهرسة الحلقة بتفافّ ٨: تسلسل النقرة → خانة الخامة
pub fn ring_index(seq: u64) -> usize {
    (seq % RING_CAP as u64) as usize
}
...
mod tests {
    ...
    #[test]
    fn فهرسة_الحلقة_تلف_على_ثمانية() {
        assert_eq!(ring_index(0), 0);
        assert_eq!(ring_index(7), 7);
        assert_eq!(ring_index(8), 0);
        assert_eq!(ring_index(9), 1);
        assert_eq!(ring_index(16), 0);
        assert_eq!(ring_index(1_000_000_007), 7);
    }
