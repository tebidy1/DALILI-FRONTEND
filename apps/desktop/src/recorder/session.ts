...
export interface RecorderSession {
  ...
  // مستهلَكة من الاختبار حصرًا — العرض الحي يقرأ أحداث المتحكّم (فحص 2026-09-21)؛ حذفها يمسّ اختبارها، فأُجّل لقرار المالك.
...
export interface SessionOptions {
  ...
  /** لحظة بناء الخطوة (مرحلة الربط ١): يُستدعى بعد دفع كل خطوةٍ للمخزن
   *  بموجزها ولقطتها (meta) — بطاقةُ لحظة الالتقاط الحيّة. اختياريّ؛
   *  غيابه سلوكٌ قائم لا يتغيّر */
  onStepBuilt?: (s: {
    title: string
    kind: string
   ...
   *  للخطوة، فتقفز بطاقة اللقطة فورًا والبكسلات تتبعها. الرقم قد
   *  يصحّحه حدثُ الخطوة لاحقًا (navigate مُدرَجة) فالبطاقة تتحدّث بالحقيقة */
  onGestureAccepted?: (n: number) => void
...
export function createRecorderSession(bridge: Bridge, opts: SessionOptions = {}): RecorderSession {
  ...
  async function buildStep(items: GestureItem[]): Promise<void> {
    ...
    if (!click) {
      // نوافذ تُفتح بضغطة حصرًا — نظريًّا لا يحدث، وإذا حدث فالبطاقة المؤقّتة تُغلق
      opts.onGestureDropped?.()
      return
    }
    ...
    // بدل خطوة أعمى بلا مرساة ولا هويّة — والبطاقة المؤقّتة تُغلق بها
    if (!waited || waited.error || !waited.facts) {
      opts.onGestureDropped?.()
      return
    }
    ...
    buffer.push(...steps)
    lastWindowKey = wkey
    // لحظة البناء للودجة الحيّة — آخر خطوةٍ دُفعت (navigate تُدرَج قبل الرئيسيّة)
    ...
    if (last) {
      opts.onStepBuilt?.({
        title: stepTitle({
          ...
          target: last.target ?? {},
          value: last.value,
          sensitive: last.sensitive ?? false,
          source: last.source,
        }),
        kind: last.kind,
  ...
  bridge.listen('sensor://input', (p) => {
    ...
    if (evt.kind === 'down') {
      ...
      pacer.open()
      windowOpen = true
      // الإشارة الفوريّة: الإيماءة قُبلت والبناء جارٍ — البطاقة تقفز الآن لا بعد
      // الالتقاط. التوقّع = المخزن + ما هو قيد البناء + ١ (النقرات المتلاحقة
      // تتقدم الرقم صحيحًا)، وقد يصحّه حدثُ الخطوة لاحقًا (navigate مُدرَجة)
      opts.onGestureAccepted?.(buffer.length + inflight + 1)
