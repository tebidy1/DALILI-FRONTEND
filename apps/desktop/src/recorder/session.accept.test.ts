...
describe('٣ج-٣ — القبول بالمحاكاة: fixture مسجَّل ⇐ دليل يمرّ zGuide', () => {
  it('مشهد Excel المعلَن يُبنى جلسةً ويخرج دليل v2 صالحًا مستقرًّا أمام الترحيل', async () => {
    ...
    // علامة الإبراز على نقرة التبويب (٣و خيار أ): حلقة فارغة مركزها نقرة
    // المالك (215,106) بنصف قطر 24 عند 96dpi — لا مستطيل العنصر (170,90,90,33)
    expect(guide.steps[3]!.screenshot).toMatchObject({
      fileId: 'frame-3',
      mark: { rect: { x: 191, y: 82, w: 48, h: 48 }, color: '#ea580c', shape: 'ellipse' },
    })
