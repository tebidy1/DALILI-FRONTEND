...
import { capabilities, CAPS_PANEL } from './capabilities'
...
describe('نصوص لوحة القدرات — مفهومة بلا شرح خارجي', () => {
  it('العنوان يجيب «ماذا أفعل بهذه القائمة؟» — يذكر المفعّل والقادم معًا', () => {
    expect(CAPS_PANEL.title).toContain('الآن')
    expect(CAPS_PANEL.title).toContain('لاحق')
  })

  it('المفتاح يفسّر الرمزين للمستخدم: ✓ يعمل — و— قادم', () => {
    expect(CAPS_PANEL.legend).toContain('✓')
    expect(CAPS_PANEL.legend).toContain('—')
    expect(CAPS_PANEL.legend.length).toBeGreaterThan(20)
  })

  it('سطر تمهيدي يوضح أن القائمة خطة صادقة لا رسالة خطأ', () => {
    expect(CAPS_PANEL.intro.length).toBeGreaterThan(20)
  })
})
