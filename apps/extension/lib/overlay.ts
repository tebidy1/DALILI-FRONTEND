...
/** I18N-01: الاتجاه والخط يتبعان اللغة الحية — دالة لا ثابت */
const styleCss = () => `
:host { all: initial; }
.layer {
  position: fixed; inset: 0; pointer-events: none; z-index: 2147483647;
  direction: ${i18nLocale() === 'en' ? 'ltr' : 'rtl'}; font-family: ${i18nLocale() === 'en' ? "'IBM Plex Sans', 'IBM Plex Sans Arabic'" : "'IBM Plex Sans Arabic'"}, 'Segoe UI', Tahoma, sans-serif;
}
.ring {
...
export function createOverlay(): Overlay {
  ...
  function mount() {
    ...
    hint.className = 'hint'
    hint.textContent = t('ext.blurDragHint')
    veil.append(sel)
    ...
    label.textContent = t('ext.recordingNow')
    mic = document.createElement('span')
    mic.className = 'mic'
    mic.textContent = t('ext.micDot')
    ...
    peek.className = 'peek'
    peek.type = 'button'
    peek.textContent = t('ext.peek')
    peek.setAttribute('aria-label', t('ext.peekAria'))
  ...
  function sync(meta: SessionMeta) {
    ...
    label.textContent = paused ? t('ext.statePaused') : t('ext.recordingNow')
    ...
    count.textContent = ` · ${t('ext.stepsCount', { count: toArabicDigits(meta.stepCount) })}`
  ...
  function showBlurCount(n: number) {
    ...
    blurCount.textContent = t('ext.blurCount', { count: toArabicDigits(n) })
