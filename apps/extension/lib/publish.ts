  ...
  for (let i = 0; i < stepCount; i++) {
    ...
    if (shotDataUrl) {
      try {
        ...
        screenshot = { missing: true, reason: t('ext.uploadFailReason') }
      }
    } else {
      screenshot = { missing: true, reason: got.missingReason ?? t('ext.noShot') }
    }
    ...
    if (memo) {
      memoDone++
      opts.onMemoProgress?.(t('ext.memoUploadProgress', { done: toArabicDigits(memoDone), total: toArabicDigits(memoTotal) }))
      try {
