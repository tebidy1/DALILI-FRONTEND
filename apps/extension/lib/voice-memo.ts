...
export function makeVoiceMemo(host: MemoHost) {
  ...
  async function stopMemo(reason: 'user' | 'cap' = 'user'): Promise<MemoStopResult> {
    const cur = active
    if (!cur) return { ok: false, errorAr: t('ext.memoNoSession') }
    clearTimer()
    active = null
