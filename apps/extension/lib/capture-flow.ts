...
import { blurFailNotice } from './blurpick'
...
export function createCaptureFlow(deps: CaptureFlowDeps) {
  ...
  // 92 سطرًا: حراسة → اختيار ما قبل اللقطة → جولة المستطيل → الالتقاط → سلّم مصادر العلامات → الطمس → الحفظ —
  // تعليقات هنا قوانين مالك تنتقل حرفيًّا مع أي استخراج (فحص 2026-09-21).
    ...
    try {
      ...
      if (manualRects.length > 0) {
        ...
        try {
          ...
          if (!deps.meta().notice) await deps.saveMeta({ notice: blurFailNotice() })
        }
      }
