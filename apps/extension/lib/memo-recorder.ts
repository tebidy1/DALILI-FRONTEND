...
export function makeMemoRecorder(deps: MemoDeps) {
  ...
  async function start(): Promise<MemoStartAck> {
    if (rec && rec.state !== 'inactive') return { ok: false, errorAr: t('ext.memoAlreadyRecording') }
    try {
      stream = await openMic({ audio: { echoCancellation: true } })
    } catch {
      return { ok: false, errorAr: t('ext.memoMicDenied') }
    }
    let r: MediaRecorder
    try {
      ...
      stream = null
      return { ok: false, errorAr: t('ext.memoUnsupported') }
    }
    blobs = []
    r.ondataavailable = (e) => {
  ...
  async function stop(): Promise<MemoStopAck> {
    if (!rec || rec.state === 'inactive') return { ok: false, errorAr: t('ext.memoNoSession') }
    const r = rec
    ...
    if (chunks.length === 0) return { ok: false, errorAr: t('ext.memoSilent') }
