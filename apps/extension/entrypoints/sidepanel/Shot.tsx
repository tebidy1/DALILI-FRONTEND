    ...
    <div className={`step-shot${zoomed ? ' zoom' : ''}`} ref={boxRef}>
      ...
      {canToggle && mark && (
        <button type="button" className="shot-toggle" aria-pressed={full} onClick={() => setFull((v) => !v)}>
          {full ? t('ext.shotFull') : t('ext.shotFullFrame')}
        </button>
      )}
    </div>
  )
...
export function PendingShot() {
  return (
    <div className="step-shot pending" role="img" aria-label={t('ext.drawingPending')}>
      <span className="pending-sheen" aria-hidden="true" />
      <span className="pending-cap">{t('ext.drawingPendingCap')}</span>
    </div>
  )
}
