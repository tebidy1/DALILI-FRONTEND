...
// I18N-01: طبقة الصفحة تتبع لغة التخزين — القراءة عند الإقلاع (والأصل عربي قبله)
void initI18nLocale()
  ...
  main() {
    ...
    overlay.onBlurDraw((rect) => {
      ...
      if (target) {
        ...
        overlay.blurHint(t('ext.blurAdded'))
      } else {
        overlay.blurHint(t('ext.blurPickHint'))
      }
    })
    ...
    const relay = (msg: Record<string, unknown>, ackTag: string) => {
      void chrome.runtime
        ...
        .catch(() => {
          window.postMessage(
            { source: 'dalili-ext', t: ackTag, ok: false, errorAr: t('ext.extNotRespondingAck') },
            '*',
          )
        })
    }
