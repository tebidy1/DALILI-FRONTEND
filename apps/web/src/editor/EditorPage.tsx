...
import { durationAr, fmtDigits, guideDurationMs, hostOf, ownerNameFromEmail, relativeTimeAr } from '../lib/format'
...
import { newBlockStep, newStepId, type BlockInsertKind } from './booklet-insert'
...
export function EditorPage({ trainAckTimeoutMs }: { trainAckTimeoutMs?: number }) {
  ...
  // TAM-01 (طلب المالك 2026-09-21): مفتاح «الطمس التلقائي» من قائمة «المزيد» —
  // رادار قراءة فقط يعمل في المحرر حصرًا: يعلّم شكوك الأنماط فوق حقائق الخطوات
  // المخزّنة، ولا يمس الالتقاط ولا الودجة ولا الخادم، وعند الإيقاف صفر كلفة.
  // الافتراضي مفعّل، والاختيار محفوظ محليًا لهذا الجهاز.
  const [autoMaskOn, setAutoMaskOn] = useState(() => localStorage.getItem('itqan.autoMaskRadar') !== '0')
  const toggleAutoMask = useCallback(() => {
    setAutoMaskOn((v) => {
      localStorage.setItem('itqan.autoMaskRadar', v ? '0' : '1')
      return !v
    })
  }, [])
  ...
  useEffect(() => {
    ...
    client
      ...
      .then((d) => {
        ...
        if (d.meta) setTags(d.meta.tags.join(getLocale() === 'en' ? ', ' : '، '))
  ...
  const applyState = useCallback((next: GuideDto | null) => {
    if (!next) return
    setGuide(next)
    setHistVer((v) => v + 1)
  }, [])

  const doUndo = useCallback(() => applyState(historyRef.current.undo()), [applyState])

  const doRedo = useCallback(() => applyState(historyRef.current.redo()), [applyState])

  const updateStep = useCallback(
    (idx: number, patch: Partial<StepDto>) => {
      const field = (['title', 'note', 'alt'] as const).find((f) => patch[f] !== undefined)
      const key = field ? `${field}:${idx}` : undefined
      commit(
  ...
  const moveStep = useCallback(
    (idx: number, dir: -1 | 1) => commit((g) => ({ ...g, steps: moveTo(g.steps, idx, idx + dir) })),
    [commit],
  )
  ...
  // TAM-01 (2026-09-21): طمس موضع الهدف من شارة الرادار بنقرة — يدخل إطار الهدف
  // نفسه في قائمة التمويه عبر مسار commit ذاته، وتسكت الشارة عن هذه الخطوة بعده
  const [autoMaskedIds, setAutoMaskedIds] = useState<ReadonlySet<string>>(new Set())
  const maskTargetAt = useCallback(
    (i: number) => {
      const st = guide?.steps[i]
      const shot = st && st.screenshot && !('missing' in st.screenshot) ? st.screenshot : null
      if (!st || !shot || !shot.mark) return
      const rect = shot.mark.rect
      commit((g) => ({
        ...g,
        steps: g.steps.map((s, j) => {
          if (j !== i) return s
          const sh = s.screenshot
          if (!sh || 'missing' in sh) return s
          return { ...s, screenshot: { ...sh, blurRects: [...sh.blurRects, rect] } }
        }),
      }))
      setAutoMaskedIds((prev) => {
  ...
  useEffect(() => {
    ...
    function onKeydown(e: KeyboardEvent) {
      ...
      if (k !== 'z' && k !== 'y') return
      e.preventDefault()
      applyState(e.shiftKey || k === 'y' ? historyRef.current.redo() : historyRef.current.undo())
    }
            ...
            <MoreMenu
              ...
              items={
                [
                  ...
                  // TAM-01 (2026-09-21): «الطمس التلقائي» — مفتاح الرادار: قراءة حقائق
                  // الخطوات وعلّم الشكوش في المحرر حصرًا؛ لا يمس الالتقاط ولا الخادم
          ...
          <div className="guide-meta">
            ...
            <span className="meta-item">
              ...
              {t(guide.kind === 'booklet' ? 'common.blocks' : 'common.steps', {
                count: fmtDigits(guide.steps.length),
              })}
            </span>
      ...
      <div className="col-stack editor-step-stack">
        ...
        {guide.kind === 'booklet' ? (
          ...
          <GuideStepList
            ...
            autoMask={autoMaskOn}
            autoMaskedIds={autoMaskedIds}
            onMaskTarget={maskTargetAt}
            onOpenBlur={() => setTool('blur')}
            onAttachShot={(i, file) => void attachShot(i, file)}
