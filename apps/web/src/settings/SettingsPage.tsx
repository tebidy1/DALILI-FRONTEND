...
import { readLocale, setLocale, type Locale } from '../lib/locale'
...
export function SettingsPage() {
  ...
  // مزامنة اللغة (I18N-01): مرآة الثيم — التطبيق فوري والخادم يتبع
  function chooseLocale(choice: Locale) {
    setLocale(choice)
    setLocaleState(choice)
    client.setMyLocale(choice).catch(() => setError(t('settings.languageSyncError')))
  }
  ...
  return (
    <div className="page settings">
      ...
      <div className="settings-grid">
        ...
        <section className="settings-card" aria-label={t('settings.language')}>
          <h2>{t('settings.language')}</h2>
          <p className="muted card-desc">{t('settings.languageDesc')}</p>
          <div className="theme-options">
            <button
              type="button"
              className={'theme-option' + (locale === 'ar' ? ' is-active' : '')}
              aria-pressed={locale === 'ar'}
              onClick={() => chooseLocale('ar')}
            >
              <b>العربية</b>
              <small>{t('settings.languageArDesc')}</small>
            </button>
            <button
              type="button"
            ...
            >
              <b>English</b>
              <small>{t('settings.languageEnDesc')}</small>
            </button>
          </div>
        </section>
