...
export function HomePage({ screen }: { screen: HomeScreen }) {
  ...
  // معرّفات أدلتي المعروضة — تحديد الكل (Ctrl+A) والاختيار بالمدى (Shift) يشتركان فيها
  const mineIds = useMemo(() => (data?.items ?? []).filter((x) => x.mine).map((x) => x.id), [data])
  /** نقر الاختيار الواحد — مع Shift يمتد التحديد بالمدى إلى أدلتي المعروضة */
  const pick = (id: string, shift: boolean) => setSel((s) => (shift ? rangeSelect(s, id, mineIds) : toggleSelect(s, id)))
