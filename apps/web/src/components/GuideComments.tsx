...
export function GuideComments(props: GuideCommentsProps) {
  ...
  /** تدفّق الإرسال المشترك للتعليق والردّ — الفراغ لا يُرسل، والفشل يُعلَن، وكل مسار يمسح مسودّته */
  async function submit(body: string, opts: { kind: CommentKind; parentId?: string }, clear: () => void) {
    if (!body) return
    setErr('')
    try {
      await onAdd(body, { ...opts, ...(canModerate ? {} : { author: name.trim() || undefined }) })
      clear()
    } catch {
  ...
  function sendReply(parentId: string) {
    void submit(replyDraft.trim(), { kind: kindOfRoot(parentId), parentId }, () => {
      setReplyDraft('')
      setReplyTo(null)
    })
  }
