...
export function StepImage({
  ...
  /**
   * قياس اللوحة المشترك: «بكسل طبيعي لكل بكسل معروض» — null إن كانت اللوحة
   * بلا مقاس بعد. القراءة وحدها مشتركة، ورياضيات كل موقع بعدها تبقى بمحلّها.
   */
  function canvasScale(): { sx: number; sy: number } | null {
    const canvas = canvasRef.current
    if (!canvas) return null
    const box = canvas.getBoundingClientRect()
    if (box.width === 0 || box.height === 0) return null
    return { sx: canvas.width / box.width, sy: canvas.height / box.height }
  }
  ...
  function onPointerDown(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (mode === 'annotate' && tool === 'text') {
      ...
      if (!scale) return
      const p = pointerPos(e)
      const { sx: scaleX, sy: scaleY } = scale
  ...
  function onPointerUp() {
    ...
    // تُرك بلا حارس صفر عمدًا وبتعليق قرار مالك — لا يُوحَّد مع canvasScale إلا بحارس مطابق للسلوك حرفيًّا.
    if (!drag) return
    const canvas = canvasRef.current!
